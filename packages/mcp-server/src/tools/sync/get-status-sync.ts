...
export const tool: Tool = {
  name: 'get_status_sync',
  description:
    "When using this tool, always use the `jq_filter` parameter to reduce the response size and improve performance.\n\nOnly omit if you're sure you don't need the data.\n\nGet sync status\n\n# Response Schema\n```json\n{\n  $ref: '#/$defs/sync_get_status_response',\n  $defs: {\n    sync_get_status_response: {\n      type: 'object',\n      properties: {\n        isStale: {\n          type: 'boolean'\n        },\n        lastSync: {\n          type: 'string',\n          format: 'date-time'\n        },\n        recordCount: {\n          type: 'integer'\n        }\n      }\n    }\n  }\n}\n```",
  inputSchema: {
    type: 'object',
    properties: {
      jq_filter: {
...
export const handler = async (client: Schools, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(await maybeFilter(jq_filter, await client.sync.getStatus()));
  } catch (error) {
    if (isJqError(error)) {
