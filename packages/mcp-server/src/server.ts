...
import { ClientOptions } from 'schools-sdk';
...
export { ClientOptions } from 'schools-sdk';
...
async function getInstructions() {
  ...
  const response = await fetch(
    readEnv('CODE_MODE_INSTRUCTIONS_URL') ?? 'https://api.stainless.com/api/ai/instructions/schools',
    {
      method: 'GET',
  ...
  if (!response.ok) {
    ...
    instructions = `
      This is the schools MCP server. You will use Code Mode to help the user perform
...
export async function initMcpServer(params: {
  ...
  let client = new Schools({
    ...{ environment: (readEnv('SCHOOLS_ENVIRONMENT') || undefined) as any },
    logger,
